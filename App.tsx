...
import { Alert, Image, ImageBackground, ImageSourcePropType, Linking, Pressable, ScrollView, Share, StyleSheet, Text, TextInput, View } from 'react-native';
...
import { blockDays, dateKey, scheduleBlockOccursOn, sortSchedule, validDate, validTime } from './src/schedule/scheduleUtils';
import { academyWeather, dailySchedule, giftForDate, isWeekend, localDateKey, npcPresence } from './src/daily/dailyEngine';
...
import { ArtCategory, ArtLayer, CustomArtAsset, JournalMood, JournalTemplate, LearningActivity, StoryScene, TaskKind } from './src/types';
...
type RootStackParamList = { AcademyTabs: undefined; Grounds:undefined; Focus: { activityId:string }; Scene: { sceneId:string }; Location:{locationId:LocationId}; BookLibrary:undefined; BookReader:{bookId:string}; Relationships:undefined; Schedule:undefined; Journal:undefined; AvatarLibrary:undefined; ArtPortal:undefined; Settings:undefined };
...
const locationArt = {
  ...
  exterior: require('./assets/grounds/academy-exterior.png'),
  frontGates: require('./assets/grounds/front-gates.png'),
  mainCourtyard: require('./assets/grounds/main-courtyard.png'),
  roseCourt: require('./assets/grounds/rose-court.png'),
  formalGardens: require('./assets/grounds/formal-gardens.png'),
  practiceGarden: require('./assets/grounds/practice-garden.png'),
  fountainCourtyard: require('./assets/grounds/fountain-courtyard.png'),
  conservatoryExterior: require('./assets/grounds/conservatory-exterior.png'),
  gardenPaths: require('./assets/grounds/garden-paths.png'),
  pathToTown: require('./assets/grounds/path-to-town.png'),
  townEntrance: require('./assets/grounds/town-entrance-locked.png'),
  lakeside: require('./assets/grounds/lakeside.png'),
};
...
function WardrobeScreen({navigation}:any) {
  const { wardrobe, student, selectedPose, selectedAvatarBust, savedLooks, customArt, buyItem, equipItem, setSelectedPose, setSelectedAvatarBust, saveCurrentLook, wearSavedLook, removeSavedLook } = useAcademyStore();
...
function FocusScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList,'Focus'>) {
  ...
  const [seconds, setSeconds] = useState(activity.minutes*60); const [running,setRunning]=useState(false); const [goal,setGoal]=useState(''); const [customSteps,setCustomSteps]=useState(''); const [reflection,setReflection]=useState('');
...
function SceneScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList,'Scene'>) {
  ...
  const expressionArt=customArt.filter(asset=>asset.enabled&&((speakingNPC&&asset.category==='npc'&&asset.targetId===speakingNPC.id)||(isPlayer&&asset.category==='avatar'&&asset.targetId==='player'))&&(asset.variant===(current.expression??'neutral')||asset.variant==='neutral'||asset.layer==='portrait'||(isPlayer&&!['expression','face'].includes(asset.layer)))).sort((a,b)=>(a.zIndex+artLayerOrder[a.layer])-(b.zIndex+artLayerOrder[b.layer]));
...
function ArtPortalScreen({navigation}:NativeStackScreenProps<RootStackParamList,'ArtPortal'>) {
  ...
  useEffect(()=>{const first=targets[0]?.id;if(first&&!targets.some(target=>target.id===targetId))setTargetId(first);const layers=artLayersByCategory[category];if(!layers.includes(layer))setLayer(layers[0]);},[category]);
  const upload=async()=>{
    try{
      ...
      addCustomArt({id:`art-${Date.now()}`,name:name.trim()||picked.name.replace(/\.[^.]+$/,''),uri:destination.uri,category,targetId,variant:variant.trim().toLowerCase()||'neutral',layer,zIndex:0,x:0,y:0,scale:1,opacity:1,enabled:true,createdAt:new Date().toISOString()});
  ...
  const nudge=(asset:CustomArtAsset,key:'x'|'y'|'scale'|'zIndex'|'opacity',amount:number)=>updateCustomArt(asset.id,{[key]:Math.round(((asset[key] as number)+amount)*100)/100});
...
function SettingsScreen({navigation}:NativeStackScreenProps<RootStackParamList,'Settings'>) {
  ...
  const exportBackup=async()=>{try{const directory=new Directory(Paths.document,'rba-backups');directory.create({idempotent:true,intermediates:true});const file=new File(directory,`rba-backup-${localDateKey()}.json`);file.write(JSON.stringify({format:'rba-backup',version:1,exportedAt:new Date().toISOString(),note:'Keep original custom artwork files backed up separately.',state:useAcademyStore.getState()},null,2),{encoding:'utf8'});await Share.share({title:'Royal Ballet Academy Backup',message:'Royal Ballet Academy backup file',url:file.uri});}catch(error){Alert.alert('Could not export backup',error instanceof Error?error.message:'Please try again.');}};
  const importBackup=async()=>{try{const result=await DocumentPicker.getDocumentAsync({type:'application/json',copyToCacheDirectory:true,multiple:false});if(result.canceled)return;const payload=JSON.parse(await new File(result.assets[0].uri).text());if(payload?.format!=='rba-backup'||!payload?.state)throw new Error('This is not a Royal Ballet Academy backup.');Alert.alert('Restore this backup?','Current local progress will be replaced. Uploaded artwork still needs its original image files on a different installation.',[{text:'Cancel',style:'cancel'},{text:'Restore',onPress:()=>{state.restoreBackup(payload.state);Alert.alert('Backup restored','Your academy data has been restored.');}}]);}catch(error){Alert.alert('Could not import backup',error instanceof Error?error.message:'Choose a valid RBA JSON backup.');}};
...
function ScheduleScreen() {
  ...
  return <Page><ScreenTitle eyebrow="Planner" title="Weekly Schedule" subtitle="Build your real schedule here. Only blocks marked for story can unlock schedule-based events."/><LocationHero source={locationArt.hallway} title="The Academy Notice Hall" subtitle="Your real commitments determine what happens throughout the academy."/>
    {!editorOpen&&<GoldButton label="Add Schedule Block" onPress={openNew}/>} 
...
export default function App() { return <SafeAreaProvider><NavigationContainer><StatusBar style="dark"/><Stack.Navigator screenOptions={{headerShown:false}}><Stack.Screen name="AcademyTabs" component={TabNavigator}/><Stack.Screen name="Grounds" component={GroundsScreen}/><Stack.Screen name="Focus" component={FocusScreen}/><Stack.Screen name="Scene" component={SceneScreen}/><Stack.Screen name="Location" component={LocationScreen}/><Stack.Screen name="BookLibrary" component={BookLibraryScreen}/><Stack.Screen name="BookReader" component={BookReaderScreen}/><Stack.Screen name="Relationships" component={RelationshipsScreen}/><Stack.Screen name="Schedule" component={ScheduleScreen}/><Stack.Screen name="Journal" component={JournalScreen}/><Stack.Screen name="AvatarLibrary" component={AvatarLibraryScreen}/><Stack.Screen name="ArtPortal" component={ArtPortalScreen}/><Stack.Screen name="Settings" component={SettingsScreen}/></Stack.Navigator></NavigationContainer></SafeAreaProvider>; }
