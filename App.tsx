...
import { ArtCategory, ArtLayer, CustomArtAsset, JournalMood, JournalTemplate, LearningActivity, Relationship, ScheduleBlock, StoryScene, TaskKind, TodaySectionId, WardrobeSlotAppearance } from './src/types';
...
function Page({ children }: { children:React.ReactNode }) {
  ...
  const goHome=()=>navigation.navigate('AcademyTabs',{screen:'Today'} as any);
  return <SafeAreaView style={styles.safe} edges={['top']}><LinearGradient colors={[colors.pale, '#F3E6E3', colors.pale]} style={StyleSheet.absoluteFill}/><View style={styles.globalNav}><Pressable style={styles.globalNavButton} onPress={()=>navigation.canGoBack()?navigation.goBack():goHome()}><Ionicons name="chevron-back" size={19} color={colors.plum}/><Text style={styles.globalNavText}>Back</Text></Pressable><Pressable style={[styles.globalNavButton,styles.globalHomeButton]} onPress={goHome}><Ionicons name="home" size={18} color="white"/><Text style={styles.globalHomeText}>Home</Text></Pressable></View><ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>{children}</ScrollView></SafeAreaView>;
}
...
function LocationHero({ source, title, subtitle, children }: { source:ImageSourcePropType; title:string; subtitle:string; children?:React.ReactNode }) {
  ...
  return <ImageBackground source={source} resizeMode={showWhole?'contain':'cover'} style={[styles.locationHero,showWhole&&styles.fullImageBackground]} imageStyle={styles.locationImage}>
    <LinearGradient colors={['transparent','rgba(44,30,36,.82)']} style={StyleSheet.absoluteFill}/>
    {children}
    <Pressable style={styles.fitImageButton} onPress={()=>setShowWhole(value=>!value)}><Ionicons name={showWhole?"expand-outline":"contract-outline"} size={15} color="white"/><Text style={styles.fitImageButtonText}>{showWhole?"Fill screen":"Show whole image"}</Text></Pressable>
...
function HomeScreen({ navigation }: any) {
  const { student, tasks, completeTask, addTask, schedule, selectedPose, selectedAvatarBust, clothingFits, roomAvatarPlacements, customArt,relationships,wardrobe,quests,dailyGiftClaims,dailyBlockCompletions,dailyPreferences,todayLayout,claimDailyGift,completeDailyBlock,setDailyPreferences,setTodaySectionVisible,moveTodaySection,resetTodayLayout } = useAcademyStore();
  const [goalTitle,setGoalTitle]=useState('');
  const [editingHome,setEditingHome]=useState(false);
  const activeAvatarPose=balletPoses.find(pose=>pose.id===selectedPose) ?? balletPoses[0];
  const activeAvatarBust=avatarBusts.find(bust=>bust.id===selectedAvatarBust) ?? avatarBusts[0];
  const equippedRoomClothes=wardrobe.filter(item=>item.equipped&&wardrobeArt[item.id]);
  ...
  const dormAvatarPlacement=roomAvatarPlacements.dorm??{x:0,y:0,scale:1};
  ...
  return <Page><TopWallet/><ScreenTitle eyebrow={isWeekend(now)?'Weekend at the Academy':'Royal Ballet Academy'} title={`Good ${now.getHours() < 12 ? 'morning' : now.getHours() < 18 ? 'afternoon' : 'evening'}, ${student.name}`} subtitle={isWeekend(now)?'A gentler academy day: relationships, gardens, reflection, and anything real on your schedule.':'Your real schedule, daily rewards, and academy events are waiting.'}/>
    <Pressable style={styles.homeEditButton} onPress={()=>setEditingHome(value=>!value)}><Ionicons name={editingHome?'checkmark-circle':'options-outline'} size={18} color={editingHome?'white':colors.plum}/><Text style={[styles.homeEditButtonText,editingHome&&styles.homeEditButtonTextActive]}>{editingHome?'Done editing':'Edit Home'}</Text></Pressable>
    {editingHome&&<PaperCard style={styles.homeEditorCard}><View style={styles.rowBetween}><View><Text style={styles.itemTitle}>Arrange your Today screen</Text><Text style={styles.small}>Hide anything you do not need and move the important stuff up.</Text></View><Pressable onPress={resetTodayLayout}><Text style={styles.positionReset}>Reset</Text></Pressable></View>{todayLayout.map((section,index)=><View key={section.id} style={styles.homeEditorRow}><Pressable style={[styles.homeVisibility,section.visible&&styles.homeVisibilityOn]} onPress={()=>setTodaySectionVisible(section.id,!section.visible)}><Ionicons name={section.visible?'eye':'eye-off'} size={16} color={section.visible?'white':colors.muted}/></Pressable><Text style={styles.homeEditorLabel}>{sectionNames[section.id]}</Text><Pressable disabled={index===0} style={[styles.homeMoveButton,index===0&&styles.homeMoveDisabled]} onPress={()=>moveTodaySection(section.id,-1)}><Ionicons name="chevron-up" size={17} color={colors.plum}/></Pressable><Pressable disabled={index===todayLayout.length-1} style={[styles.homeMoveButton,index===todayLayout.length-1&&styles.homeMoveDisabled]} onPress={()=>moveTodaySection(section.id,1)}><Ionicons name="chevron-down" size={17} color={colors.plum}/></Pressable></View>)}</PaperCard>}
    {todayLayout.filter(section=>section.visible).map(section=><React.Fragment key={section.id}>{sectionContent[section.id]}</React.Fragment>)}
  </Page>;
}
...
function LocationScreen({route,navigation}:NativeStackScreenProps<RootStackParamList,'Location'>) {
  ...
  const [showWholeImage,setShowWholeImage]=useState(true);
  ...
  const selectedPose=useAcademyStore(state=>state.selectedPose);
  const selectedAvatarBust=useAcademyStore(state=>state.selectedAvatarBust);
  const wardrobe=useAcademyStore(state=>state.wardrobe);
  const clothingFits=useAcademyStore(state=>state.clothingFits);
  const roomAvatarPlacements=useAcademyStore(state=>state.roomAvatarPlacements);
  const updateRoomAvatarPlacement=useAcademyStore(state=>state.updateRoomAvatarPlacement);
  const [dormFeature,setDormFeature]=useState<'music'|null>(null);
  const [adjustingRoomAvatar,setAdjustingRoomAvatar]=useState(false);
  ...
  const roomAvatarPose=balletPoses.find(pose=>pose.id===selectedPose)??balletPoses[0];
  const roomAvatarBust=avatarBusts.find(bust=>bust.id===selectedAvatarBust)??avatarBusts[0];
  const roomAvatarClothes=wardrobe.filter(item=>item.equipped&&wardrobeArt[item.id]);
  const roomAvatarArt=customArt.filter(asset=>asset.category==='avatar'&&asset.targetId==='player'&&asset.enabled&&(!asset.itemId||wardrobe.some(item=>item.id===asset.itemId&&item.equipped)));
  const roomAvatarPlacement=roomAvatarPlacements[route.params.locationId]??{x:0,y:0,scale:1};
  const nudgeRoomAvatar=(key:'x'|'y'|'scale',amount:number)=>{const next=roomAvatarPlacement[key]+amount;const bounds=key==='scale'?[.45,2.2]:[-150,150];updateRoomAvatarPlacement(route.params.locationId,{[key]:Math.round(Math.max(bounds[0],Math.min(bounds[1],next))*100)/100});};
  ...
  return <SafeAreaView style={styles.locationPage}><ImageBackground source={locationBackground?{uri:locationBackground.uri}:room.art} style={[StyleSheet.absoluteFill,showWholeImage&&styles.fullImageBackground]} resizeMode={showWholeImage?"contain":"cover"}><PlacedArt assets={roomDecor}/><LinearGradient colors={['rgba(34,24,29,.02)','rgba(89,54,71,.11)','rgba(64,34,48,.74)']} style={StyleSheet.absoluteFill}/><View style={styles.locationTop}><Pressable style={styles.roundBack} onPress={()=>navigation.goBack()}><Ionicons name="chevron-back" size={25} color="white"/></Pressable><Pill tone="gold">ROYAL BALLET ACADEMY</Pill></View><Pressable style={styles.locationFitButton} onPress={()=>setShowWholeImage(value=>!value)}><Ionicons name={showWholeImage?"expand-outline":"contract-outline"} size={16} color="white"/><Text style={styles.fitImageButtonText}>{showWholeImage?"Fill screen":"Show whole image"}</Text></Pressable>{route.params.locationId==='dorm'&&<Pressable style={styles.avatarMoveButton} onPress={()=>setAdjustingRoomAvatar(value=>!value)}><Ionicons name={adjustingRoomAvatar?'checkmark':'move-outline'} size={16} color="white"/><Text style={styles.fitImageButtonText}>{adjustingRoomAvatar?'Done':'Move avatar'}</Text></Pressable>}
    {route.params.locationId==='dorm'?<>
      <View pointerEvents="none" style={[styles.dormPlayerAvatar,{transform:[{translateX:roomAvatarPlacement.x},{translateY:roomAvatarPlacement.y},{scale:roomAvatarPlacement.scale}]}]}><Image source={roomAvatarPose.art} resizeMode="contain" style={styles.roomAvatarPose}/><Image source={roomAvatarBust.art} resizeMode="contain" style={styles.roomAvatarFace}/>{roomAvatarClothes.map(item=>{const fit=clothingFits[item.id]??{x:0,y:0,scale:1};return <Image key={item.id} source={wardrobeArt[item.id].source} resizeMode="contain" style={[styles.roomWearableOverlay,wardrobeArt[item.id].style,{transform:[{translateX:fit.x*.38},{translateY:fit.y*.38},{scale:fit.scale}]}]}/>})}<PlacedArt assets={roomAvatarArt} style={styles.roomCustomAvatarLayers}/></View>
      <Pressable accessibilityLabel="Open music box" style={[styles.dormObjectHotspot,{left:'8%',top:'29%'}]} onPress={()=>setDormFeature('music')}><Ionicons name="musical-notes" size={18} color="white"/><Text style={styles.dormHotspotLabel}>Music Box</Text></Pressable>
      <Pressable accessibilityLabel="Open wardrobe" style={[styles.dormObjectHotspot,{right:'7%',top:'31%'}]} onPress={()=>navigation.navigate('AcademyTabs',{screen:'Wardrobe'} as any)}><Ionicons name="shirt" size={18} color="white"/><Text style={styles.dormHotspotLabel}>Wardrobe</Text></Pressable>
      <Pressable accessibilityLabel="Open schedule" style={[styles.dormObjectHotspot,{left:'10%',top:'48%'}]} onPress={()=>navigation.navigate('Schedule')}><Ionicons name="calendar" size={18} color="white"/><Text style={styles.dormHotspotLabel}>Planner</Text></Pressable>
      <Pressable accessibilityLabel="Open books" style={[styles.dormObjectHotspot,{right:'9%',top:'49%'}]} onPress={()=>navigation.navigate('BookLibrary')}><Ionicons name="library" size={18} color="white"/><Text style={styles.dormHotspotLabel}>Books</Text></Pressable>
      <Pressable accessibilityLabel="Go to dormitory hall" style={[styles.dormObjectHotspot,{left:'40%',top:'18%'}]} onPress={()=>openExit('dormHallway')}><Ionicons name="exit-outline" size={18} color="white"/><Text style={styles.dormHotspotLabel}>Hall</Text></Pressable>
      {adjustingRoomAvatar&&<View style={styles.roomAvatarAdjustPanel}><View style={styles.rowBetween}><View><Text style={styles.musicBoxEyebrow}>AVATAR PLACEMENT</Text><Text style={styles.itemTitle}>Place yourself in the room</Text></View><Pressable onPress={()=>updateRoomAvatarPlacement(route.params.locationId,{x:0,y:0,scale:1})}><Text style={styles.positionReset}>Reset</Text></Pressable></View><View style={styles.roomAvatarMiniControls}><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('x',-4)}><Ionicons name="chevron-back" size={18} color={colors.plum}/></Pressable><View><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('y',-4)}><Ionicons name="chevron-up" size={18} color={colors.plum}/></Pressable><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('y',4)}><Ionicons name="chevron-down" size={18} color={colors.plum}/></Pressable></View><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('x',4)}><Ionicons name="chevron-forward" size={18} color={colors.plum}/></Pressable><View><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('scale',-.05)}><Ionicons name="remove" size={18} color={colors.plum}/></Pressable><Pressable style={styles.roomAvatarMiniButton} onPress={()=>nudgeRoomAvatar('scale',.05)}><Ionicons name="add" size={18} color={colors.plum}/></Pressable></View></View><Text style={styles.roomAvatarAdjustHint}>This placement is saved for this room.</Text></View>}
      {dormFeature==='music'&&<View style={styles.musicBoxPanel}><View style={styles.rowBetween}><View><Text style={styles.musicBoxEyebrow}>YOUR MUSIC BOX</Text><Text style={styles.musicBoxTitle}>Academy Collection</Text></View><Pressable style={styles.musicBoxClose} onPress={()=>setDormFeature(null)}><Ionicons name="close" size={20} color={colors.plum}/></Pressable></View><Text style={styles.musicBoxText}>A little collection for the room. Audio can be wired in as you add tracks; the hotspot now has a real home instead of being spiritually present somewhere offscreen.</Text><View style={styles.musicTrackRow}><Ionicons name="musical-note" size={17} color={colors.gold}/><Text style={styles.musicTrackName}>Waltz of the Flowers</Text><Pill tone="gold">owned</Pill></View><View style={styles.musicTrackRow}><Ionicons name="lock-closed" size={15} color={colors.muted}/><Text style={styles.musicTrackName}>Academy Evening Waltz</Text><Text style={styles.musicTrackLocked}>unlock later</Text></View></View>}
...
function WardrobeScreen({navigation}:any) {
  const { wardrobe,student,selectedPose,selectedAvatarBust,clothingFits,savedLooks,customArt,buyItem,equipItem,setSelectedPose,setSelectedAvatarBust,updateClothingFit,saveCurrentLook,wearSavedLook,removeSavedLook,updateCustomArt } = useAcademyStore();
  ...
  const [adjustingClothingId,setAdjustingClothingId]=useState<string|null>(null);
  ...
  const selectedClothing=wardrobe.find(item=>item.id===selectedClothingId&&wardrobeArt[item.id]);
  const nudgeClothing=(key:'x'|'y'|'scale',amount:number)=>{if(!selectedClothing)return;const current=clothingFits[selectedClothing.id]??{x:0,y:0,scale:1};const next=current[key]+amount;const bounds=key==='scale'?[.35,2.5]:[-120,120];updateClothingFit(selectedClothing.id,{[key]:Math.round(Math.max(bounds[0],Math.min(bounds[1],next))*100)/100});};
  ...
  return <Page><TopWallet/><ScreenTitle eyebrow="Dormitory Dressing Room" title="Avatar & Room Editor" subtitle="Tap what you want and see it immediately."/>
    ...
    {editorMode==='avatar'?<>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} decelerationRate="fast" contentContainerStyle={styles.itemCarousel}>{categoryItems.map(item=>{const art=wardrobeArt[item.id];return <Pressable key={item.id} style={[styles.clothingCard,item.equipped&&styles.clothingCardActive]} onPress={()=>{setSelectedClothingId(item.id);item.owned?equipItem(item.id):!item.unlock&&student.coins>=item.price?buyItem(item.id):undefined;}}><View style={styles.clothingThumb}>{art?<Image source={art.source} resizeMode="contain" style={styles.clothingThumbImage}/>:<View style={[styles.clothingSwatch,{backgroundColor:item.color}]}/>}</View><Text numberOfLines={2} style={styles.clothingName}>{item.name}</Text><Text style={styles.clothingMeta}>{item.equipped?'Wearing':item.owned?'Tap to wear':item.unlock?'Locked':`${item.price} coins`}</Text></Pressable>})}</ScrollView>
      {selectedClothing&&selectedClothing.owned&&<Pressable style={styles.adjustItemButton} onPress={()=>setAdjustingClothingId(current=>current===selectedClothing.id?null:selectedClothing.id)}><Ionicons name={adjustingClothingId===selectedClothing.id?'checkmark-circle':'options-outline'} size={18} color={colors.plum}/><View style={styles.flex}><Text style={styles.itemTitle}>{adjustingClothingId===selectedClothing.id?'Done adjusting':`Adjust ${selectedClothing.name}`}</Text><Text style={styles.small}>Fit is saved to this item and reused in every outfit.</Text></View></Pressable>}
      {selectedClothing&&selectedClothing.owned&&adjustingClothingId===selectedClothing.id&&<PaperCard style={styles.positionCard}><View style={styles.rowBetween}><View><Text style={styles.itemTitle}>{selectedClothing.name}</Text><Text style={styles.small}>Move or resize this item only. Changes save automatically.</Text></View><Pressable onPress={()=>updateClothingFit(selectedClothing.id,{x:0,y:0,scale:1})}><Text style={styles.positionReset}>Reset fit</Text></Pressable></View><View style={styles.positionControls}><View style={styles.directionPad}><Pressable style={[styles.directionButton,styles.directionUp]} onPress={()=>nudgeClothing('y',-2)}><Ionicons name="chevron-up" size={20} color={colors.plum}/></Pressable><Pressable style={[styles.directionButton,styles.directionLeft]} onPress={()=>nudgeClothing('x',-2)}><Ionicons name="chevron-back" size={20} color={colors.plum}/></Pressable><View style={styles.directionCenter}/><Pressable style={[styles.directionButton,styles.directionRight]} onPress={()=>nudgeClothing('x',2)}><Ionicons name="chevron-forward" size={20} color={colors.plum}/></Pressable><Pressable style={[styles.directionButton,styles.directionDown]} onPress={()=>nudgeClothing('y',2)}><Ionicons name="chevron-down" size={20} color={colors.plum}/></Pressable></View><View style={styles.scaleControls}><Pressable style={styles.scaleButton} onPress={()=>nudgeClothing('scale',-.04)}><Ionicons name="remove" size={22} color={colors.plum}/><Text style={styles.scaleText}>Smaller</Text></Pressable><Pressable style={styles.scaleButton} onPress={()=>nudgeClothing('scale',.04)}><Ionicons name="add" size={22} color={colors.plum}/><Text style={styles.scaleText}>Bigger</Text></Pressable></View></View></PaperCard>}
      ...
      <View style={styles.roomEditorStage}><Image source={locationArt.dorm} resizeMode="contain" style={StyleSheet.absoluteFill}/><PlacedArt assets={roomDecor}/>{roomDecor.length===0&&<View style={styles.roomEditorEmpty}><Ionicons name="bed-outline" size={30} color={colors.gold}/><Text style={styles.body}>Your empty dorm is ready for furniture.</Text></View>}</View>
...
function ScheduleScreen() {
  ...
  const [scheduleView,setScheduleView]=useState<'calendar'|'weekly'>('calendar');
  const monthLabel=visibleMonth.toLocaleDateString(undefined,{month:'long',year:'numeric'});
  const firstMondayIndex=(visibleMonth.getDay()+6)%7;
  const calendarStart=new Date(visibleMonth.getFullYear(),visibleMonth.getMonth(),1-firstMondayIndex);
  const calendarDays=Array.from({length:42},(_,index)=>{const value=new Date(calendarStart);value.setDate(calendarStart.getDate()+index);return value;});
  const blocksForDate=(value:Date)=>schedule.filter(block=>scheduleBlockOccursOn(block,value)).sort(sortSchedule);
  const selectedBlocks=blocksForDate(selectedDate);
  const moveMonth=(delta:number)=>{const next=new Date(visibleMonth.getFullYear(),visibleMonth.getMonth()+delta,1);setVisibleMonth(next);setSelectedDate(new Date(next.getFullYear(),next.getMonth(),1));};
  const jumpToday=()=>{const now=new Date();setVisibleMonth(new Date(now.getFullYear(),now.getMonth(),1));setSelectedDate(new Date(now.getFullYear(),now.getMonth(),now.getDate()));};
  const upcomingDays=Array.from({length:30},(_,index)=>{const value=new Date(today);value.setDate(today.getDate()+index);return {date:value,blocks:blocksForDate(value)};}).filter(item=>item.blocks.length>0).slice(0,8);
  ...
  return <Page><ScreenTitle eyebrow="Planner" title="Schedule & Calendar" subtitle="See today, this month, and what is coming next."/>
    <View style={styles.scheduleViewTabs}><Pressable style={[styles.scheduleViewTab,scheduleView==='calendar'&&styles.scheduleViewTabActive]} onPress={()=>setScheduleView('calendar')}><Ionicons name="calendar-outline" size={18} color={scheduleView==='calendar'?'white':colors.plum}/><Text style={[styles.scheduleViewTabText,scheduleView==='calendar'&&styles.scheduleViewTabTextActive]}>Month</Text></Pressable><Pressable style={[styles.scheduleViewTab,scheduleView==='weekly'&&styles.scheduleViewTabActive]} onPress={()=>setScheduleView('weekly')}><Ionicons name="list-outline" size={18} color={scheduleView==='weekly'?'white':colors.plum}/><Text style={[styles.scheduleViewTabText,scheduleView==='weekly'&&styles.scheduleViewTabTextActive]}>Recurring</Text></Pressable></View>
    {scheduleView==='calendar'&&<><PaperCard style={styles.calendarCard}><View style={styles.calendarHeader}><Pressable style={styles.calendarArrow} onPress={()=>moveMonth(-1)}><Ionicons name="chevron-back" size={21} color={colors.plum}/></Pressable><View style={styles.calendarTitleWrap}><Text style={styles.calendarMonth}>{monthLabel}</Text><Pressable onPress={jumpToday}><Text style={styles.calendarTodayLink}>Jump to today</Text></Pressable></View><Pressable style={styles.calendarArrow} onPress={()=>moveMonth(1)}><Ionicons name="chevron-forward" size={21} color={colors.plum}/></Pressable></View><View style={styles.calendarWeekHeader}>{shortDays.map(day=><Text key={day} style={styles.calendarWeekday}>{day}</Text>)}</View><View style={styles.calendarGrid}>{calendarDays.map(day=>{const key=dateKey(day);const inMonth=day.getMonth()===visibleMonth.getMonth();const selected=key===dateKey(selectedDate);const isToday=key===dateKey(today);const count=blocksForDate(day).length;return <Pressable key={key} style={[styles.calendarDay,!inMonth&&styles.calendarDayOutside,selected&&styles.calendarDaySelected]} onPress={()=>{setSelectedDate(day);if(!inMonth)setVisibleMonth(new Date(day.getFullYear(),day.getMonth(),1));}}><Text style={[styles.calendarDayNumber,isToday&&styles.calendarTodayNumber,selected&&styles.calendarDayNumberSelected]}>{day.getDate()}</Text>{count>0&&<View style={[styles.calendarDot,selected&&styles.calendarDotSelected]}><Text style={[styles.calendarDotText,selected&&styles.calendarDotTextSelected]}>{count}</Text></View>}</Pressable>})}</View></PaperCard>
    <SectionLabel action={selectedDate.toLocaleDateString(undefined,{month:'short',day:'numeric'})}>Selected Day</SectionLabel>{selectedBlocks.length===0?<PaperCard><Text style={styles.itemTitle}>Nothing scheduled</Text><Text style={styles.body}>This day is open.</Text></PaperCard>:selectedBlocks.map(block=><PaperCard key={`${dateKey(selectedDate)}-${block.id}`} style={styles.selectedDayBlock}><View style={styles.row}><View style={styles.dailyTime}><Text style={styles.dailyTimeText}>{block.start}</Text></View><View style={styles.flex}><Text style={styles.itemTitle}>{block.title}</Text><Text style={styles.small}>{block.end} · {block.location} · {block.kind}</Text>{block.notes&&<Text style={styles.body}>{block.notes}</Text>}</View></View></PaperCard>)}
    <SectionLabel action="next 30 days">Coming Up</SectionLabel>{upcomingDays.length===0?<PaperCard><Text style={styles.body}>No commitments in the next 30 days.</Text></PaperCard>:upcomingDays.map(item=><Pressable key={dateKey(item.date)} onPress={()=>{setSelectedDate(item.date);setVisibleMonth(new Date(item.date.getFullYear(),item.date.getMonth(),1));}}><PaperCard style={styles.upcomingDayCard}><View style={styles.upcomingDate}><Text style={styles.upcomingDow}>{item.date.toLocaleDateString(undefined,{weekday:'short'}).toUpperCase()}</Text><Text style={styles.upcomingNum}>{item.date.getDate()}</Text></View><View style={styles.flex}><Text style={styles.itemTitle}>{item.blocks[0].title}{item.blocks.length>1?` +${item.blocks.length-1} more`:''}</Text><Text style={styles.small}>{item.date.toLocaleDateString(undefined,{month:'long',day:'numeric'})} · {item.blocks[0].start} · {item.blocks[0].location}</Text></View><Ionicons name="chevron-forward" size={18} color={colors.muted}/></PaperCard></Pressable>)}
    </>}
...
const styles=StyleSheet.create({
  ...
  globalNav:{minHeight:54,paddingHorizontal:spacing.md,paddingVertical:7,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:1,borderBottomColor:colors.goldPale,backgroundColor:'rgba(255,249,243,.97)'},
  globalNavButton:{minHeight:38,minWidth:78,paddingHorizontal:12,borderRadius:radius.pill,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5,borderWidth:1,borderColor:colors.goldPale,backgroundColor:colors.paper},
  globalNavText:{fontSize:12,fontWeight:'800',color:colors.plum},
  globalHomeButton:{backgroundColor:colors.plum,borderColor:colors.plum,minWidth:92},
  globalHomeText:{fontSize:12,fontWeight:'900',color:'white'},
  ...
  scheduleSpotlight:{minHeight:82,borderRadius:radius.lg,borderWidth:2,borderColor:colors.gold,backgroundColor:'#FFF7ED',padding:13,marginBottom:12,flexDirection:'row',alignItems:'center',gap:11,shadowColor:colors.shadow,shadowOpacity:.12,shadowRadius:10,elevation:3},
  scheduleSpotlightIcon:{width:48,height:48,borderRadius:16,backgroundColor:colors.plum,alignItems:'center',justifyContent:'center'},
  scheduleSpotlightEyebrow:{fontSize:9,fontWeight:'900',letterSpacing:1.1,color:colors.rose},
  scheduleSpotlightTitle:{fontFamily:'Georgia',fontSize:17,fontWeight:'700',color:colors.plum,marginTop:2},
  scheduleSpotlightMeta:{fontSize:10,lineHeight:14,color:colors.muted,marginTop:3},
  scheduleSpotlightCount:{minWidth:46,alignItems:'center',justifyContent:'center',gap:2},
  scheduleSpotlightCountText:{fontFamily:'Georgia',fontSize:15,fontWeight:'700',color:colors.plum},
  ...
  homeRoomAvatar:{position:'absolute',left:'39%',bottom:25,width:116,height:180,zIndex:3},
  dormPlayerAvatar:{position:'absolute',left:'42%',bottom:'24%',width:112,height:176,zIndex:4},
  roomAvatarPose:{position:'absolute',left:0,top:0,width:'100%',height:'100%'},
  roomAvatarFace:{position:'absolute',left:'16%',top:'2%',width:'68%',height:'43%'},
  roomWearableOverlay:{position:'absolute'},
  roomCustomAvatarLayers:{position:'absolute',left:0,top:0,width:'100%',height:'100%'},
  dormObjectHotspot:{position:'absolute',minWidth:72,minHeight:48,borderRadius:16,backgroundColor:'rgba(105,73,84,.88)',borderWidth:1,borderColor:colors.gold,paddingHorizontal:9,paddingVertical:6,alignItems:'center',justifyContent:'center',zIndex:7,shadowColor:colors.shadow,shadowOpacity:.22,shadowRadius:6,elevation:4},
  dormHotspotLabel:{fontSize:8,fontWeight:'900',color:'white',marginTop:2},
  dormLocationPanel:{bottom:18,paddingVertical:13},
  musicBoxPanel:{position:'absolute',left:22,right:22,top:'24%',zIndex:12,borderRadius:radius.lg,borderWidth:2,borderColor:colors.gold,backgroundColor:'rgba(255,249,243,.98)',padding:15,shadowColor:colors.shadow,shadowOpacity:.28,shadowRadius:18,elevation:10},
  musicBoxEyebrow:{fontSize:9,fontWeight:'900',letterSpacing:1.2,color:colors.rose},
  musicBoxTitle:{fontFamily:'Georgia',fontSize:21,fontWeight:'700',color:colors.plum,marginTop:2},
  musicBoxText:{fontSize:11,lineHeight:16,color:colors.muted,marginTop:8,marginBottom:10},
  musicBoxClose:{width:34,height:34,borderRadius:17,backgroundColor:'#F7E1E7',alignItems:'center',justifyContent:'center'},
  musicTrackRow:{minHeight:42,borderTopWidth:1,borderTopColor:colors.goldPale,flexDirection:'row',alignItems:'center',gap:8},
  musicTrackName:{flex:1,fontFamily:'Georgia',fontSize:12,fontWeight:'700',color:colors.plum},
  musicTrackLocked:{fontSize:9,fontWeight:'800',color:colors.muted},
  ...
  scheduleViewTabs:{flexDirection:'row',gap:8,marginBottom:12,backgroundColor:'#F7EDE7',padding:5,borderRadius:radius.pill,borderWidth:1,borderColor:colors.goldPale},
  scheduleViewTab:{flex:1,minHeight:42,borderRadius:radius.pill,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:7},
  scheduleViewTabActive:{backgroundColor:colors.plum},
  scheduleViewTabText:{fontSize:12,fontWeight:'900',color:colors.plum},
  scheduleViewTabTextActive:{color:'white'},
  calendarCard:{padding:10,marginBottom:14,borderWidth:2,borderColor:colors.gold},
  calendarHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:10},
  calendarArrow:{width:42,height:42,borderRadius:15,alignItems:'center',justifyContent:'center',backgroundColor:'#F9EEF1',borderWidth:1,borderColor:colors.goldPale},
  calendarTitleWrap:{alignItems:'center',flex:1},
  calendarMonth:{fontFamily:'Georgia',fontSize:21,fontWeight:'700',color:colors.plum},
  calendarTodayLink:{fontSize:10,fontWeight:'900',color:colors.rose,marginTop:3},
  calendarWeekHeader:{flexDirection:'row',marginBottom:4},
  calendarWeekday:{width:'14.2857%',textAlign:'center',fontSize:9,fontWeight:'900',color:colors.muted,letterSpacing:.4},
  calendarGrid:{flexDirection:'row',flexWrap:'wrap'},
  calendarDay:{width:'14.2857%',aspectRatio:1,minHeight:42,padding:4,alignItems:'center',justifyContent:'center',borderRadius:12},
  calendarDayOutside:{opacity:.35},
  calendarDaySelected:{backgroundColor:colors.plum},
  calendarDayNumber:{fontSize:13,fontWeight:'800',color:colors.ink},
  calendarDayNumberSelected:{color:'white'},
  calendarTodayNumber:{textDecorationLine:'underline',color:colors.rose},
  calendarDot:{minWidth:17,height:17,borderRadius:9,paddingHorizontal:4,marginTop:2,backgroundColor:'#F1D9D8',alignItems:'center',justifyContent:'center'},
  calendarDotSelected:{backgroundColor:'white'},
  calendarDotText:{fontSize:8,fontWeight:'900',color:colors.plum},
  calendarDotTextSelected:{color:colors.plum},
  selectedDayBlock:{borderLeftWidth:4,borderLeftColor:colors.gold,marginBottom:8},
  upcomingDayCard:{flexDirection:'row',alignItems:'center',gap:12,marginBottom:8},
  upcomingDate:{width:48,alignItems:'center',justifyContent:'center',paddingVertical:5,borderRadius:14,backgroundColor:'#F9EEF1'},
  upcomingDow:{fontSize:8,fontWeight:'900',color:colors.rose},
  upcomingNum:{fontFamily:'Georgia',fontSize:20,fontWeight:'700',color:colors.plum},
  ...
  avatarFaceLayer:{position:'absolute',width:'25%',height:'27%',top:8,left:'37.5%'},
  wearableOverlay:{position:'absolute'},
  fullImageBackground:{backgroundColor:'#F0E2D2',borderWidth:8,borderColor:'#C7A66B'},
  fitImageButton:{position:'absolute',right:10,top:10,zIndex:20,backgroundColor:'rgba(46,36,40,.72)',borderRadius:radius.pill,paddingHorizontal:10,paddingVertical:7,flexDirection:'row',alignItems:'center',gap:5},
  locationFitButton:{position:'absolute',right:16,top:74,zIndex:30,backgroundColor:'rgba(46,36,40,.72)',borderRadius:radius.pill,paddingHorizontal:10,paddingVertical:7,flexDirection:'row',alignItems:'center',gap:5},
  avatarMoveButton:{position:'absolute',left:16,top:74,zIndex:30,backgroundColor:'rgba(105,73,84,.86)',borderRadius:radius.pill,paddingHorizontal:10,paddingVertical:7,flexDirection:'row',alignItems:'center',gap:5,borderWidth:1,borderColor:colors.gold},
  roomAvatarAdjustPanel:{position:'absolute',left:16,right:16,top:116,zIndex:25,borderRadius:radius.md,borderWidth:2,borderColor:colors.gold,backgroundColor:'rgba(255,249,243,.97)',padding:12},
  roomAvatarMiniControls:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,marginTop:10},
  roomAvatarMiniButton:{width:38,height:38,borderRadius:19,backgroundColor:'#F9EEF1',borderWidth:1,borderColor:colors.goldPale,alignItems:'center',justifyContent:'center',marginVertical:2},
  roomAvatarAdjustHint:{fontSize:9,color:colors.muted,textAlign:'center',marginTop:7},
  fitImageButtonText:{fontSize:9,fontWeight:'800',color:'white'},
  ...
  adjustItemButton:{marginTop:8,borderRadius:radius.md,borderWidth:1,borderColor:colors.goldPale,backgroundColor:'#FFF9F3',padding:11,flexDirection:'row',alignItems:'center',gap:9},
  ...
  homeEditButton:{alignSelf:'flex-end',marginBottom:10,borderRadius:radius.pill,borderWidth:1,borderColor:colors.gold,backgroundColor:'#FFF9F3',paddingHorizontal:12,paddingVertical:8,flexDirection:'row',alignItems:'center',gap:6},
  homeEditButtonText:{fontSize:10,fontWeight:'900',color:colors.plum},
  homeEditButtonTextActive:{color:colors.plum},
  homeEditorCard:{marginBottom:12,borderWidth:2,borderColor:colors.gold},
  homeEditorRow:{minHeight:44,flexDirection:'row',alignItems:'center',gap:8,borderTopWidth:1,borderTopColor:colors.goldPale,paddingVertical:6},
  homeVisibility:{width:32,height:32,borderRadius:16,backgroundColor:'#EEE5E2',alignItems:'center',justifyContent:'center'},
  homeVisibilityOn:{backgroundColor:colors.sage},
  homeEditorLabel:{flex:1,fontSize:11,fontWeight:'800',color:colors.plum},
  homeMoveButton:{width:32,height:32,borderRadius:16,backgroundColor:'#F9EEF1',borderWidth:1,borderColor:colors.goldPale,alignItems:'center',justifyContent:'center'},
  homeMoveDisabled:{opacity:.28},
